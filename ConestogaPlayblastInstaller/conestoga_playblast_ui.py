...
def generate_playblast_report(playblast_path, report_dir=None, include_screenshot=True):
    ...
    file_size = os.path.getsize(playblast_path) / (1024 * 1024)  # MB
...
def is_ffmpeg_available():
    """Check if FFmpeg is available and provide feedback."""
    ffmpeg_path = get_ffmpeg_path()
    if not ffmpeg_path or not os.path.exists(ffmpeg_path):
        print("FFmpeg not found. Set path in Settings tab.")
        return False
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"FFmpeg found but failed to execute: {result.stderr}")
            return False
    except Exception as e:
...
def update_shot_mask_scale(scale):
    ...
    mask_transform = cmds.ls(f"{presets.MASK_PREFIX}transform", type="transform")
    if mask_transform:
...
def update_shot_mask_text_color(color):
    ...
    text_material = cmds.ls(f"{presets.MASK_PREFIX}TextMaterial", type="lambert")
    if text_material:
