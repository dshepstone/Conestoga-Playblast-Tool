...
# Import get_frame_rate from the correct module
from conestoga_playblast import get_frame_rate
...
class PlayblastDialog(QtWidgets.QDialog):
    ...
    def reset_playblast_settings(self):
        ...
        if result == QtWidgets.QMessageBox.Yes:
            # Reset to defaults
            self.cameraComboBox.setCurrentText("<Active>")
            ...
            # Reset h264 preset
            self.h264PresetComboBox.setCurrentText(DEFAULT_H264_PRESET)
            
            # Clear output path and filename
            scene_name = os.path.basename(cmds.file(q=True, sn=True, shn=True)).split('.')[0] or "untitled"
            self.filenameLineEdit.setText(f"{scene_name}_{{camera}}")
            
            # Reset output directory to project/movies
            workspace_dir = cmds.workspace(query=True, rd=True)
            movies_dir = os.path.normpath(os.path.join(workspace_dir, "movies"))
            self.outputDirLineEdit.setText(movies_dir)
...
# Helper functions for option vars
def save_option_var(name, value):
    if isinstance(value, bool):
        ...
        if len(value) > 0:
            if isinstance(value[0], bool):
                cmds.optionVar(iv=(name, int(value[0])))
                for v in value[1:]:
                    cmds.optionVar(iva=(name, int(v)))
            elif isinstance(value[0], int):
                cmds.optionVar(iv=(name, value[0]))
                for v in value[1:]:
                    cmds.optionVar(iva=(name, v))
            elif isinstance(value[0], float):
                cmds.optionVar(fv=(name, value[0]))
                for v in value[1:]:
                    cmds.optionVar(fva=(name, v))
            elif isinstance(value[0], str):
                cmds.optionVar(sv=(name, value[0]))
                for v in value[1:]:
                    cmds.optionVar(sva=(name, v))
    elif isinstance(value, dict):
        # Convert to JSON string
        json_str = json.dumps(value)
        cmds.optionVar(sv=(name, json_str))

def load_option_var(name, default=None):
    if not cmds.optionVar(exists=name):
        return default
        
    value = cmds.optionVar(q=name)
    
    # Try to parse as JSON if it's a string and looks like a dictionary or list
